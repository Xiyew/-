...
#define PROID			"HURwaH1HK2"

#define ACCESS_KEY		"OTN3THEyV2NJOUFDbEhEZnZrYTR5SFBWeU9LUkJWYU0="
...
_Bool OneNet_DevLink(void)
{
	...
	_Bool status = 1;
	/*填写产品相关信息:产品ID,密钥信息,设备名称从而生成Token*/
...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	/*空气质量*/
	memset(text, 0, sizeof(text));
	sprintf(text, "\"PPM\":{\"value\":%.1lf},", AD_Data.Sensor_Data.MQ135);
	strcat(buf, text);
	/*噪声*/
	memset(text, 0, sizeof(text));
	sprintf(text, "\"db\":{\"value\":%d},", AD_Data.Sensor_Data.LM2904);
	strcat(buf, text);
	/*亮度*/
	memset(text, 0, sizeof(text));
	sprintf(text, "\"luminance\":{\"value\":%d},", AD_Data.Sensor_Data.MH);
	strcat(buf, text);
	/*亮度*/
	memset(text, 0, sizeof(text));
	sprintf(text, "\"light\":{\"value\":%d},", AD_Data.Sensor_Data.light);
	strcat(buf, text);
	/*LED状态*/
	memset(text, 0, sizeof(text));
	...
	/*温度*/	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"temp\":{\"value\":%.1f},", DHT11_Data.temp);
	strcat(buf, text);
	/*电流*/	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"amps\":{\"value\":%d},", AD_Data.Sensor_Data.ACS712);
	strcat(buf, text);
	/*电压*/	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"volt\":{\"value\":%.2f},", AD_Data.Sensor_Data.volt);
	strcat(buf, text);
	/*湿度*/
	memset(text, 0, sizeof(text));
	sprintf(text, "\"humi\":{\"value\":%.1f}", DHT11_Data.humi);
	strcat(buf, text);
	...
	switch(type)
	{
		case MQTT_PKT_PUBLISH:																//接收的Publish消息
			...
			if(result == 0)
			{
				...
				raw_json = cJSON_Parse(req_payload);	//将原始字符串信息转换为CJSON格式
				...
				cJSON_Delete(raw_json);	//将解析完成的CJSON删除
			}
